...
/**
 * fc comment, builder pattern. Builder for configuring a new SslContext for
 * creation.
 */
public final class SslContextBuilder {
     ...
     * Creates a builder for new server-side {@link SslContext}.
     *
     * @param keyCertChainInputStream an input stream for an X.509 certificate chain
     *                                in PEM format
     * @param keyInputStream          an input stream for a PKCS#8 private key in
     *                                PEM format
     ...
     * @param key          a PKCS#8 private key
     * @param keyCertChain the X.509 certificate chain
     * @param keyPassword  the password of the {@code keyFile}, or {@code null} if
     ...
     * Creates a builder for new server-side {@link SslContext}.
     *
     * If you use {@link SslProvider#OPENSSL} or {@link SslProvider#OPENSSL_REFCNT}
     * consider using {@link OpenSslX509KeyManagerFactory} or
    ...
    /**
     * The {@link SslContext} implementation to use. {@code null} uses the default
     * one.
     */
    ...
    /**
     * Sets the {@link KeyStore} type that should be used. {@code null} uses the
     * default one.
     */
    ...
    /**
     * The SSLContext {@link Provider} to use. {@code null} uses the default one.
     * This is only used with {@link SslProvider#JDK}.
     */
     ...
     * file should contain an X.509 certificate collection in PEM format.
     * {@code null} uses the system default.
     */
    public SslContextBuilder trustManager(File trustCertCollectionFile) {
        try {
            ...
            throw new IllegalArgumentException("File does not contain valid certificates: " + trustCertCollectionFile,
                    e);
        }
    }
    ...
    /**
     * Trusted certificates for verifying the remote endpoint's certificate. The
     * input stream should contain an X.509 certificate collection in PEM format.
     * {@code null} uses the system default.
     */
    ...
    /**
     * Trusted manager for verifying the remote endpoint's certificate. {@code null}
    ...
    /**
     * Identifying certificate for this host. {@code keyCertChainFile} and
     * {@code keyFile} may be {@code null} for client contexts, which disables
    ...
    /**
     * Identifying certificate for this host. {@code keyCertChainInputStream} and
     * {@code keyInputStream} may be {@code null} for client contexts, which
    ...
    /**
     * Identifying certificate for this host. {@code keyCertChain} and {@code key}
     * may be {@code null} for client contexts, which disables mutual
     * authentication.
     *
     * @param key          a PKCS#8 private key
     ...
     * @param keyInputStream          an input stream for a PKCS#8 private key in
     *                                PEM format
     * @param keyPassword             the password of the {@code keyInputStream}, or
     *                                {@code null} if it's not password-protected
     */
    public SslContextBuilder keyManager(InputStream keyCertChainInputStream, InputStream keyInputStream,
            String keyPassword) {
     ...
     * @param keyPassword  the password of the {@code key}, or {@code null} if it's
    ...
    /**
     * Identifying manager for this host. {@code keyManagerFactory} may be
     * {@code null} for client contexts, which disables mutual authentication. Using
     * a {@link KeyManagerFactory} is only supported for {@link SslProvider#JDK} or
     * {@link SslProvider#OPENSSL} / {@link SslProvider#OPENSSL_REFCNT} if the used
     * openssl version is 1.0.1+. You can check if your openssl version supports
     * using a {@link KeyManagerFactory} by calling
     * {@link OpenSsl#supportsKeyManagerFactory()}. If this is not the case you must
     * use {@link #keyManager(File, File)} or
     * {@link #keyManager(File, File, String)}.
     *
     * If you use {@link SslProvider#OPENSSL} or {@link SslProvider#OPENSSL_REFCNT}
     * consider using {@link OpenSslX509KeyManagerFactory} or
     * {@link OpenSslCachingX509KeyManagerFactory}.
     ...
     * will be applied to the ciphers before use. If {@code ciphers} is
     * {@code null}, then the default cipher suites will be used.
     */
     ...
     * implementations support OCSP stapling and an exception will be thrown upon
     * {@link #build()}.
     *
     ...
     * <p>
     * If {@link #sslProvider(SslProvider)} is set to
     * {@link SslProvider#OPENSSL_REFCNT} then the caller is responsible for
     * releasing this object, or else native memory may leak.
     */
    public SslContext build() throws SSLException {
        if (forServer) {
            return SslContext.newServerContextInternal(provider, sslContextProvider, trustCertCollection,
                    ...
                    sessionCacheSize, sessionTimeout, clientAuth, protocols, startTls, enableOcsp, keyStoreType);
        } else {
            return SslContext.newClientContextInternal(provider, sslContextProvider, trustCertCollection,
                    trustManagerFactory, keyCertChain, key, keyPassword, keyManagerFactory, ciphers, cipherFilter, apn,
                    protocols, sessionCacheSize, sessionTimeout, enableOcsp, keyStoreType);
        }
    }
